...
const startEditWebhook = (webhook: ChannelWebhook): void => {
    ...
    editWebhookDescription.value = webhook.description
...
const updateWebhook = async (): Promise<void> => {
    ...
    try {
        await channelCtrl.webhooks.update(editWebhook.value.uuid, {}, editWebhookForm.value)
    } catch (error: any) {
...
const createWebhook = async (): Promise<void> => {
    if (!webhookName.value) {
        return toastCtrl.add('Name is required', 'error')
    }
    if (!webhookDescription.value) {
        return toastCtrl.add('Description is required', 'error')
    }
    try {
        await channelCtrl.webhooks.create({}, newWebhookForm.value)
    } catch (error: any) {
        toastCtrl.add(error.message, 'error')
        return
    }
...
const deleteWebhookAvatarFile = async (room_file_uuid: string) => {
    try {
    ...
    editWebhook.value = null
    await roomSelector.reinitWebhooks()
    toastCtrl.add('Webhook Avatar deleted', 'success')
}
    ...
    <div v-if="displayWebhooks === true" style="z-index:2;"
        ...
        <div class="w-96">
            <div class="w-96 bg-black text-white p-4 rounded-md">
                ...
                <div class="mb-3">
                    <div v-if="isAdmin">
                        <div v-if="newWebhook" class="flex gap-1">
                            <form ref="newWebhookForm" @submit.prevent="createWebhook()">
                                <input type="hidden" name="uuid" :value="v4uuid()" />

                                <div>
                                    <label for="name" class="font-bold block">Name</label>
                                    <small class="mb-1 block">
                                        Used to identify the webhook.
                                    </small>
                                    <input v-model="webhookName" placeholder="Name" name="name"
                                ...
                                <div>
                                    ...
                                    <small class="mb-1 block">
                                        Describe the webhook to help users understand its purpose.
                                    </small>
                                    <textarea v-model="webhookDescription" placeholder="Description" name="description"
                                        class="w-full p-2 border border-gray-300 rounded-md text-black h-24" />
                                </div>

                                <div class="mb-3">
                                    <label class="font-bold block">Avatar</label>
                                    <small class="mb-1 block text-sm">
                                        Upload an image avatar for the webhook.
                                    </small>
                                ...
                                <div class="mb-3">
                                    <label class="font-bold block">Channel</label>
                                    <small class="mb-1 block text-sm">
                                        Select the channel to send messages to.
                                    </small>
                                    <select v-model="channelUuid" name="channel_uuid"
                                ...
                                <div class="flex gap-1">
                                    ...
                                    <button type="button" @click="newWebhook = false"
                        ...
                        <div v-else-if="editWebhook" class="flex gap-1">
                            <form ref="editWebhookForm" @submit.prevent="updateWebhook()">
                                <input type="hidden" name="uuid" :value="editWebhook.uuid" />
                                ...
                                <div>
                                    ...
                                    <input v-model="editWebhookName" placeholder="Name" name="name"
                                        class="w-full p-2 border border-gray-300 rounded-md text-black" />
                                </div>
                                ...
                                <div>
                                    ...
                                    <textarea v-model="editWebhookDescription" placeholder="Description"
                                        name="description"
                                        class="w-full p-2 border border-gray-300 rounded-md text-black h-24" />
                                </div>
                                ...
                                <div>
                                    ...
                                    <small class="mb-1 block text-sm">
                                        Upload an image to represent the webhook.
                                    </small>
                                    <input type="file" placeholder="avatar" name="file"
                                        class="w-full p-2 border border-gray-300 rounded-md text-white" />
                                    <div v-if="editWebhook?.room_file" class="flex items-center gap-2 mt-2">
                                        <img :src="editWebhook.room_file.src" class="w-12 h-12 rounded-full" />
                                        <button @click="deleteWebhookAvatarFile(editWebhook.room_file.uuid)"
                                            type="button" title="Delete Avatar"
                                ...
                                <div class="mb-3">
                                    ...
                                    <select v-model="editWebhookchannelUuid" name="channel_uuid"
                                        class="w-full text-black rounded-md p-2 border border-gray-300">
                                        <option value="">Select Channel</option>
                                        <option v-for="channel in channels" :key="channel.uuid" :value="channel.uuid">
                                            {{ channel.name }}
                                ...
                                <div class="flex gap-1">
                                    ...
                                    <button type="button" @click="editWebhook = null"
                                        class="p-2 bg-slate-500 hover:bg-slate-600 rounded-md text-white">
                                        Cancel
                                    </button>
                                </div>
                ...
                <ul class="flex flex-col gap-3 mb-3 break-all">
                    <li v-for="webhook in webhooks" :key="webhook.uuid">
                        <div class="text-white w-full shadow-md bg-slate-700 rounded-md p-3">
                            <div class="flex items-center gap-3 border-b border-gray-600 pb-2 mb-2">
                                <div>
                            ...
                            <div class="text-xs border-b border-gray-600 pb-2 mb-2">
                                <div class="mb-1">
                                    Description:
                                </div>
                                <div>
                            ...
                            <div class="text-xs border-b border-gray-600 pb-2 mb-2">
                                ...
                                <div>
                                    {{ getChannel(webhook.channel_uuid)?.name }}
                                </div>
                            </div>
                            ...
                            <div class="text-xs border-b border-gray-600 pb-2 mb-2">
                                ...
                                <div>
                                    <a :href="getLink(webhook)" target="_blank" class="text-indigo-200 hover:underline">
                                        {{ getLink(webhook) }}
                                    </a>
                                </div>
                            </div>
                            ...
                            <div class="text-xs border-b border-gray-600 pb-2 mb-2">
                                ...
                                <div>
                                    <div class="w-full text-xs mb-1 bg-slate-500 p-3 rounded-md break-all">
                                        curl -X POST
                                        {{ getLink(webhook) }}
